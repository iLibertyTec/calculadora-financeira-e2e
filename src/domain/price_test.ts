...
import { calculatePriceSchedule } from "./price.ts";
...
Deno.test("calculatePriceSchedule validates input", () => {
  assertThrows(
    () => calculatePriceSchedule({ principal: 0, monthlyRate: 0.01, termMonths: 12 }),
    Error,
  ...
  assertThrows(
    () => calculatePriceSchedule({ principal: 1000, monthlyRate: 0.01, termMonths: 0 }),
    Error,
